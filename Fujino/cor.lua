...
function init_gear_sets()
  ...
  sets.weapons.DeathPenalty = {range="デスペナルティ", ammo="ライヴブレット"}
  sets.weapons.Fomalhaut = {range="フォーマルハウト", ammo="ライヴブレット"}
  sets.weapons.Armageddon = {range="アルマゲドン", ammo="ライヴブレット"}
  sets.weapons.TPBonus = {range={ name="アナーキー+2", augments={'Delay:+60','TP Bonus +1000',}}, ammo="ライヴブレット"}
...
function disp_roll_info(spell)
  ...
  --windower.add_to_chat(4, spell.name .. ' [' ..desc ..'] Lucky=' .. lucky .. ' '.. ' Unluck='.. unlucky)
end

function mogmaster(job)
  --send_command('input /echo '..job..';')
end